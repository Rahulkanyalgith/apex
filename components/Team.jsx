    ...
    return (
        <div className="flex flex-col items-center justify-center w-full min-h-screen gap-20 py-20 overflow-hidden bg-gradient-to-b to-white via-green-50 from-white">
            <div className="flex flex-col justify-end w-11/12 leading-none tracking-tighter">
                <h1 className="text-[4rem] sm:text-[5rem] md:text-[7rem] lg:text-[9rem] font-black text-gray-700">Our Team</h1>
                <h1 className="text-[1.2rem] md:text-[2rem] font-medium text-gray-600">The Futures Of Apex’s {year} 🎀</h1>
            </div>
                ...
                <Marquee loop={0} delay={0} speed={20} >
                    {TemaData.map((data, index) => (
                        <div className="relative flex flex-col h-full gap-6 p-6 ml-10 overflow-hidden duration-300 border rounded-xl w-fit" key={index}>
                            ...
                            <h1 className="text-[2rem] font-black text-gray-600">{data.name}</h1>
                        </div>
                    ))}
                </Marquee>
            </div>
